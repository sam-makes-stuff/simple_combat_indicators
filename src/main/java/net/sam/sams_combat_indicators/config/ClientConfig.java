...
public class ClientConfig {
    ...
    static {
        ...
        NUMBER_DURATION = BUILDER.comment("Damage number duration (in ticks)").define("number_duration", 15);

        INITIAL_HIT_TIME = BUILDER.comment("Duration damage numbers flash a different color and size initially (set to 0 to disable) (in ticks)").define("separate_initial_hit_time", 10);

        START_NUMBER_COLOR_R = BUILDER.comment("Start damage number color red value (0-255)").define("start_number_color_r", 255);
        START_NUMBER_COLOR_G = BUILDER.comment("Start damage number color green value (0-255)").define("start_number_color_g", 102);
        START_NUMBER_COLOR_B = BUILDER.comment("Start damage number color blue value (0-255)").define("start_number_color_b", 0);

        END_NUMBER_COLOR_R = BUILDER.comment("End damage number color red value (0-255)").define("end_number_color_r", 255);
        END_NUMBER_COLOR_G = BUILDER.comment("End damage number color green (0-255)").define("end_number_color_g", 255);
        END_NUMBER_COLOR_B = BUILDER.comment("End damage number color blue value (0-255)").define("end_number_color_b", 0);
